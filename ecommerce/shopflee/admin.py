...
class UpdateCashInResource(resources.ModelResource):
    ...
    class Meta:
        model = UpdateCashIn
        fields = ('id', 'date', 'income_source', 'cash_in', 'status', 'remark')  # Include 'id' in the fields for import
...
class UpdateCashInAdmin(ImportExportModelAdmin, ExportActionMixin):
    ...
    list_display = ['date', 'income_source', 'cash_in', 'status', 'remark']
    search_fields = ['income_source', 'date']
    list_filter = ['status']
    ordering = ['-date']
    ...
    delete_all_data_action.short_description = "DELETE ALL RECORD"
...
class UpdateCashOutResource(resources.ModelResource):
    ...
    class Meta:
        model = UpdateCashOut
        fields = ('id', 'date', 'expense_source', 'cash_out', 'status', 'remark')  # Include 'id' in the fields for import
    def before_import_row(self, row, **kwargs):
        # Disable date validation to allow importing all data
        pass
...
class UpdateCashOutAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:  # Check if it's a new record (not being edited)
            self.fields['processed'].initial = True  # Set processed checkbox to True by default
            self.fields['processed'].widget = forms.HiddenInput()  # Hide processed checkbox
...
class UpdateCashOutAdmin(ImportExportModelAdmin, ExportActionMixin):
    ...
    list_display = ['date', 'expense_source', 'cash_out', 'status', 'remark', 'formatted_priority']
    search_fields = ['expense_source', 'date']
    list_filter = ['status']
    ordering = ['-date']
...
class SummaryAdmin(ImportExportModelAdmin, ExportActionMixin):
    ...
    def view_transaction_details(self, request, object_id, *args, **kwargs):
        ...
        details_html = render_to_string('admin/shopflee/summary/transaction_details_table.html', {
    ...
    change_list_template = 'admin/shopflee/summary/change_list.html'
    ...
    def render_change_form(self, request, context, add=False, change=False, form_url='', obj=None):
        ...
        if transaction_details:
            ...
            transaction_details_html = render_to_string('admin/shopflee/summary/transaction_details_table.html', transaction_details)
